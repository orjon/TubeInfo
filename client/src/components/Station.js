...
const Station = ({
  ...
  const stationIndex = stations.findIndex(station => props.match.params.url === station.url)
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    const intervalStatuses = setInterval(() => {setStatusesExpired(calledFrom)}, (maxStatusesAge*1000));
    const intervalArrivals = setInterval(() => {getStationArrivals({calledFrom, stationId})}, (maxArrivalsAge*1000));
    return () => {
  ...
  if (station) {
    ...
    if (station.id === '940GZZLUHOH') {
      address = 'Harrow on the Hill Station, London Underground Ltd, Station Rd, Harrow HA1 1BB'
...
export default connect(mapStateToProps, { getStatuses, setStatusesExpired, getStations, getStationArrivals })(Station);