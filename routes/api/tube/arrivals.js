...
router.get('/:stationId', async (req,res) => {
  let stationIndex =  serverState.stations.findIndex(station => station.id === req.params.stationId)
  let station = serverState.stations[stationIndex]

  // console timer
  let now = moment()
  console.log('=================================================')
  console.log(moment(now).format('HH:mm:ss'),'GET request /api/tube/arrivals/'+station.name)
  ...
  try {
    let response = await axios.get(`https://api.tfl.gov.uk/stoppoint/${station.id}/arrivals?${TFLAccess}`, {
