...
router.get('/', async (req,res) => {
  ...
  let now = moment()
  console.log('=================================================')
  console.log(moment(now).format('HH:mm:ss'),'GET request /api/tube/stations/')
  ...
  if (serverState.stationsTimeStamp ) {
    ...
    const updatedToday = moment(now).isSame(serverState.stationsTimeStamp,'day')
  ...
  for (let i=0; i< statuses.length; i++ ){
    ...
    try {
      let response = await axios.get(`https://api.tfl.gov.uk/line/${lineId}/stoppoints?tflOperatedNationalRailStationsOnly=false&${TFLAccess}`, {
      ...
      let newStations = thisLineStations.filter(newStation => !allStations.find(station => newStation.id === station.id))
      let unchangedStations = allStations.filter(station => !thisLineStations.find( lineStation => station.id === lineStation.id))
      let updateStations = allStations.filter(station => thisLineStations.find( lineStation => station.id === lineStation.id))
      ...
      lineStations = [...lineStations, { id: lineId, stations: thisLineStations} ]


    } catch (error) {
      console.log('*********** ERROR! **************')
      console.log('GET api/tube/stations error at:', lineId)
      console.log(error)
      console.log('*********** ERROR! **************')
    }
  }
